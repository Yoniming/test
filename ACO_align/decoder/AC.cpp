...
void
ac_decoder_init(ac_decoder *acd, const char *fn, int * line_maxsize, int *max_1,int *min_1, int * T)
{
	int i;
	...
	{
		fread(line_maxsize, sizeof(int), 1, acd->fp);
		fread(max_1, sizeof(int), 1, acd->fp);
		fread(min_1, sizeof(int), 1, acd->fp);
		fread(T, sizeof(int), 1, acd->fp);
